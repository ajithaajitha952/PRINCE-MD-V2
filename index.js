...
const Taira = TairaConnect({
    ...
    auth: state,
    browser: ['PRINCE-MD-V2', 'safari', '3.3'],
    ...
    getMessage: async (key) => {
                ...
                return { conversation: "PRINCE-MD-V2" };
    }
})
...
Taira.setStatus = (status) => {
   ...
   Taira.ev.on('connection.update', async (update) => {
        ...
        if (connection === "open" || update.receivedPendingNotifications == "true")  {
          ...
          console.log("Welcome to PRINCE-MD-V2 ✨");
          const userName = Taira.user.name ? Taira.user.name : global.BotName;
          console.log(' PRINCE-MD-V2 User Info');
...
Taira.ev.on('call', async (message) => {
    ...
    let BlockMsg= await Taira.sendMessage(mkv2.from, { text: `PRINCE-MD-V2 user can't take calls for now,please wait for him to reply instead`})
    await sleep(8000)
...
Taira.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require("./PRINCE-MD-V2.js")(Taira, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
